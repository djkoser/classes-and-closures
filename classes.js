...
class Machine {
  ...
  makeWidgets(increase) {
    this.widgets_made_count += increase;
    this.wear_and_tear_count += Math.floor(increase / 50);
  }
  fixMachine() {
  ...
  reboot() {
    let target = this;
    return function () {
      if (target.wear_and_tear_count !== 0) {
        target.wear_and_tear_count -= 10;
      };
      target.needs_reboot = false;
    };
  };
};
